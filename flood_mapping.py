...
AOI       Bago / Yangon region, Myanmar
Pre-flood  2026-05-25  (+/-DATE_WINDOW days)
Post-flood 2026-06-06  (+/-DATE_WINDOW days)
...
PRE_FLOOD_DATE  = "2026-05-25"
POST_FLOOD_DATE = "2026-06-06"
DATE_WINDOW     = 6       # +/- days to search for imagery around each target date
...
# 30 m keeps file sizes manageable; decrease for sharper output (larger HTML).
EXPORT_SCALE = 30
...
def download_as_geotiff(image: ee.Image, name: str,
    ...
    """
    Download a single-band GEE image to TEMP_DIR/<name> using EPSG:4326
    so that rasterio reads geographic (lon/lat) bounds directly.
    """
    os.makedirs(TEMP_DIR, exist_ok=True)
    path = os.path.join(TEMP_DIR, name)
    if not os.path.exists(path):
        print(f"  Downloading {name} at {scale} m/px ...")
        geemap.ee_export_image(
            image, filename=path,
            scale=scale, region=aoi,
...
def build_map(overlays: dict, flood_km2: float, output: str) -> None:
    ...
    ">
      ...
      <span style="color:#666;font-size:11px">Myanmar &mdash; Bago / Yangon</span>
